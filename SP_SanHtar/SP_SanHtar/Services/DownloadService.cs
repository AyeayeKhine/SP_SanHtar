    ...
    public class DownloadService : IDownloadService
    {
        ...
        public async Task DownloadFileAsync(string url, IProgress<double> progress, CancellationToken token)
        {
            try
            {
                ...
                // Step 2 : Filename
                string genFileName = DateTime.Now.ToString("ddMMyyyyHHmmss");
                var fileName = response.Content.Headers?.ContentDisposition?.FileName ?? genFileName+".zip";
                ...
                Helpers.Constants.videoUrl = filePath;
